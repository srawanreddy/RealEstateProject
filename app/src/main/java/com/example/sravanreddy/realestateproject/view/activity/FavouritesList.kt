...
class FavouritesList : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        recyclerView = findViewById(R.id.RecyclerView_Favourites_list)
        recyclerView!!.layoutManager = LinearLayoutManager(this@FavouritesList)
        getFireBaseData()
    }

    private fun getFireBaseData() : List<PropertyModel> {
        var propertyModels = ArrayList<PropertyModel>()
        Firebase.setAndroidContext(this@FavouritesList)
    ...
    private fun setRecyclerView(propertyModels : List<PropertyModel>) {
        var favouriteListAdapter = FavouriteListAdapter(propertyModels, this@FavouritesList)
